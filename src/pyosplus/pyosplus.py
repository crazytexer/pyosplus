...
def write_html_dir_tree(
    ...
    `shrunk_depth: int = -1`
      The depth (i.e., hierarchy level) from which all directories
      to be shrunk (collapsed) in HTML.
      The depth of `directory` equals 0.
      `shrunk_depth = -1` means that no directories are shrunk
      except those in `shrunk_dirs` (if any).
    `shrunk_text: str = " &lt;...&gt;"`
      The text to be put next to a shrunk directory name.
    `ignored_dirs: str | list[str] = []`
      Paths to the directories to be ignored. Unlike the shrunk
      directories, `ignored_dirs` are not visible in HTML at all.
    `print_exts: bool = True`
      Print file extensions (`True`) or not (`False`).
    `ignored_exts: str | list[str] = []`
      Extensions of files to be ignored in HTML. Such files
      are not visible in HTML at all.
    `before: str = None`
    ...
    if not isinstance(ignored_exts, list):
        ignored_exts = [ignored_exts]
    if (directory in shrunk_dirs) or (shrunk_depth == 0):
    ...
    code = ('''<!DOCTYPE html>\n<html>\n<head><meta charset="utf-8"></head>'''
            '''\n<body>\n<samp>\n''')
    before = before if before is not None else escape(directory)
    if before:
        code += before + "\n<hr>\n"
    code += "<br>\n".join(result) + "\n"
    after = after if after is not None else escape(directory)
    if after:
        code += "<hr>\n" + after + "\n"
    code += '''</samp>\n</body>\n</html>\n'''
